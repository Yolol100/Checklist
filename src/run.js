...
function qualifyArtifactEntry(entry, runId) {
  ...
  return {
    ...entry,
    source: artifactName
      ? `github-actions-artifact:${artifactName}/${entry.source}`
      : `artifacts/runs/${runId}/${entry.source}`
  };
}
  ...
  for (const run of result.browser_evidence?.runs || []) {
    ...
    registry.push({
      ...
      sha256: sha256Json({
        ...
        websocket_requests: run.websocket_requests
      }),
  ...
  return runs
    .map((run) => {
      ...
      return {
        ...
        websocket_requests: [...(run.websocket_requests || [])].sort()
      };
    })
...
const result = await runChecklist(target.href, request.level, artifactDir, request.target_environment);
...
const payload = {
  ...
  final_evidence_contract: {
    ...
    policy_input: "policy/queue/<evaluation_id>.json",
    output: "results/formal/<evaluation_id>.json",
